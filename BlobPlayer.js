...
// Slow pacing + hidden discoveries + hidden collectibles + horizontal looping

const VIEW_W = 800;
const VIEW_H = 480;
const WORLD_W = 6000;
const WORLD_H = 800; // vertical travel allowed
...
function draw() {
  ...
  updateCamera();      // camera centers instantly
...
function drawCosmicBackground() {
  ...
  for (let y = 0; y < height; y++) {
    ...
    let c = inter < 0.5
      ? lerpColor(topColor, midColor, inter * 2)
      : lerpColor(midColor, bottomColor, (inter - 0.5) * 2);
    stroke(c);
    line(0, y, width, y);
  }
}
...
class Constellation {
  ...
  update() {
    ...
    if (!this.found && dist(screenX, screenY, width / 2, height / 2) < 40) this.found = true;
  ...
  draw() {
    if (!this.found) {
      ...
      noFill(); stroke(255, this.alpha); strokeWeight(2);
...
class Collectible {
  ...
  draw() {
    ...
    noStroke(); fill(255, 255, 100, this.alpha);
...
class BlobPlayer {
  ...
  update() {
    ...
    // constrain vertical only (horizontal loops)
    this.x = (this.x + WORLD_W) % WORLD_W;
  ...
  draw() {
    noStroke();
    fill(120, 180, 255, 40); circle(this.x, this.y, this.r * 3);
...
class ShootingStar {
  constructor(x, y) {
    this.x = x; this.y = y; this.vx = random(8,12); this.vy = random(-1,1); this.alpha = 255;
  }
